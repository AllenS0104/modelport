...
import { PLATFORM_NAME } from '@/components/platform-brand'
...
;(function initSystemBranding() {
  try {
    ...
    const apply = (name: string) => {
      document.title = `${PLATFORM_NAME} · ${name}`
      const metaTitle = document.querySelector(
        'meta[name="title"]'
      ) as HTMLMetaElement | null
      if (metaTitle) {
        metaTitle.setAttribute('content', `${PLATFORM_NAME} · ${name}`)
      }
    }
    // Cache-first
