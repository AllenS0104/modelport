  ...
  if (variant === 'inline') {
    return (
      <a
        ...
        className={cn(
          'text-foreground inline-flex min-w-0 flex-col items-start rounded-md px-1.5 transition-colors outline-none select-none',
          'hover:bg-accent focus-visible:ring-ring/40 focus-visible:ring-2'
        )}
      >
        <PlatformBrand />
        <span className='modelport-upstream'>
          <span className='flex size-3 shrink-0 items-center justify-center overflow-hidden rounded-md'>
            <img
              src={logo}
              alt={t('Logo')}
          ...
          <span className='max-w-[12rem] truncate'>Powered by {name}</span>
        </span>
      </a>
    )
  }
